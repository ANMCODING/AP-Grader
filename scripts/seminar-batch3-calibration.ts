/**
 * Batch 3 calibration — data/seminar/batch3-calibration/*.txt
 * Run: npx tsx scripts/seminar-batch3-calibration.ts
 */
import fs from "node:fs";
import path from "node:path";
import { gradeSeminarPaper } from "@/lib/seminar";

const ROOT = path.join(process.cwd(), "data/seminar/batch3-calibration");
const TARGETS = JSON.parse(
  fs.readFileSync(
    path.join(process.cwd(), "data/seminar/batch3-targets.json"),
    "utf8",
  ),
) as {
  tolerance: number;
...
function main(): void {
  ...
  if (missing === TARGETS.papers.length) {
    console.error("\nNo batch3 paper files found.");
    process.exit(1);
  }
  console.log(
    `\nAll present Batch 3 papers within total band (±${TARGETS.tolerance}).`,
  );
}
