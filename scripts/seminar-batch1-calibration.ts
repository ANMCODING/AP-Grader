/**
 * Batch 1 calibration check — data/seminar/batch1-calibration/*.txt
 * Run: npx tsx scripts/seminar-batch1-calibration.ts
 */
import fs from "node:fs";
import path from "node:path";
import { gradeSeminarPaper } from "@/lib/seminar";

const ROOT = path.join(process.cwd(), "data/seminar/batch1-calibration");
const TARGETS = JSON.parse(
  fs.readFileSync(
    path.join(process.cwd(), "data/seminar/batch1-targets.json"),
    "utf8",
  ),
) as {
  tolerance: number;
...
function main(): void {
  ...
  if (missing === TARGETS.papers.length) {
    console.error("\nNo batch1 paper files found. Add .txt files from PDFs.");
    process.exit(1);
  }
  console.log(
    `\nAll present Batch 1 papers within total band (±${TARGETS.tolerance}).`,
  );
}
