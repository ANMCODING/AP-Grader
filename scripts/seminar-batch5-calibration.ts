/**
 * Batch 5 calibration — data/seminar/batch5-calibration/*.txt
 * Run: npx tsx scripts/seminar-batch5-calibration.ts
 */
import fs from "node:fs";
import path from "node:path";
import { gradeSeminarPaper } from "@/lib/seminar";

const ROOT = path.join(process.cwd(), "data/seminar/batch5-calibration");
const TARGETS = JSON.parse(
  fs.readFileSync(
    path.join(process.cwd(), "data/seminar/batch5-targets.json"),
    "utf8",
  ),
) as {
  tolerance: number;
...
function main(): void {
  ...
  console.log(`\nBatch 5: ${passed}/${TARGETS.papers.length - missing} pass`);
  if (missing > 0) {
